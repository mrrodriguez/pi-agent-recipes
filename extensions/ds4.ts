...
const DS4_DIR = process.env.DS4_DIR || join(homedir(), ".pi", "ds4");
const SETTINGS_FILE = join(DS4_DIR, "settings.json");
const KV_DIR = process.env.DS4_KV_DIR || join(DS4_DIR, "kv");
const SUPPORT_DIR = process.env.DS4_RUNTIME_DIR || join(DS4_DIR, "support");
...
function registerDs4Skill(_pi: ExtensionAPI): void {
        /* Skill directory not copied in this manual setup */
}
...
export default function (pi: ExtensionAPI) {
        if (!process.env.DS4_ENABLED) return;
        console.log("DeepSeek V4 Flash (ds4.c) extension enabled");
        runtimeDisposed = false;	shuttingDown = false;
